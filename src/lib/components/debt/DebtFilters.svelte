<script>
  ...
  import { setDebtSearch, setDebtSort, debts } from '$lib/stores/debts';
...
<div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
  ...
  <div class="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-1">
    {#each sortOptions as option}
      ...
      {@const active = $debts.sortBy === option.value}
      <button
        type="button"
        onclick={() => setDebtSort(option.value)}
